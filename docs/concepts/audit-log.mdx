...
By default, read-only requests (e.g. listing minions or jobs) are **not** written to the audit log. The `AUDIT_AUDIT_READS` env var (`config.py`, default `false`) can be set to `true` to change this. When enabled, read operations that pass a permission check are also recorded. This produces a much higher log volume and is intended for compliance environments that require full access logging.
