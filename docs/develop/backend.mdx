...
The flow: `EventStreamConsumer` (started by `RuntimeConfig` when the stream is enabled) calls `SaltAPIClient.stream_events()`, runs each raw event through `normalize_event()`, persists the keepers via `service.persist_event()`, and publishes them to the `EventHub`. The SSE route subscribes to the hub to push events to browsers. See [How Halite Works](/concepts/how-it-works#the-live-event-stream) for the architecture and the single-worker constraint.
