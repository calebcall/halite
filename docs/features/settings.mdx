...
<ParamField body="log_format" type="string" default="json">
  The backend log format. Accepted values: `json` (structured JSON lines, best for log aggregators) or `text` (human-readable). Changing this setting and saving it does **not** hot-reload the logger — restart the container for the new format to take effect.
</ParamField>

## How live reload works

When you save any settings section (Salt, Pollers, or Logging — except logging), Halite calls `runtime.reload(db)`:
